...
public class Timer
{
	...
	public float getFuncX(){
		return Utils.getFuncXByTime(timer,st,ed);
	}
	public float getNLF(){
		return Utils.getNLinearValueByTime(timer,st,ed);
	}
	public float getSF(){
		return Utils.getSinValueByTime(timer,st,ed);
	}
	public int render(float dt){
		timer+=dt;
		timer=Utils.limit(timer,0,3600000);
		int size=ts.size();
